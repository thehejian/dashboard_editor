...
test.describe('个性化 - 告警聚合配置按钮', () => {
  ...
  test('聚合开关可切换', async ({ page }) => {
    await page.locator('.ant-tabs-tab', { hasText: '通知偏好' }).click()
    await page.waitForTimeout(300)
    const toggle = page.locator('.ant-switch:visible').last()
    await toggle.waitFor({ state: 'visible' })
    const isChecked = await toggle.evaluate(el => el.classList.contains('ant-switch-checked'))
    await toggle.click({ force: true })
    await page.waitForTimeout(300)
    const isChecked2 = await toggle.evaluate(el => el.classList.contains('ant-switch-checked'))
    expect(isChecked).not.toEqual(isChecked2)
  })
})
