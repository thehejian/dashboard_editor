<template>
  <div>
    <div class="page-header"><h3>数据库账号</h3></div>
...
const columns = [
  ...
  { title: '所属实例', dataIndex: 'instance', key: 'instance' },
...
onMounted(async () => {
  loading.value = true
  try {
    const res = await fetch('/api/cmdb/accounts?account_type=DB&sort=id&order=ASC')
