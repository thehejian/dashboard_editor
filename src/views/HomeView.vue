    ...
    <div class="alarm-analysis-page">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'action'">
              <div class="action-btns">
                <a-tooltip :title="record.category === '容量类' ? '一键自愈' : '查看详情'">
                ...
                <a-tooltip title="查看详情"><button class="icon-btn" @click.stop="openAlarmDetail(record)"><i class="fa-solid fa-eye"></i></button></a-tooltip>
              </div>
            </template>
          </template>
        </a-table>
      ...
      <div class="aa-table-card">
        ...
        <div class="aa-app-grid" v-if="alarmApps.length">
          <div v-for="app in alarmApps" :key="app.name" class="aa-app-card" :class="'aa-app-' + app.status">
...
const homeTab = ref(route.path === '/aiops' ? 'aiops' : route.path.startsWith('/alarm') ? 'alarm' : 'home')

function switchTab(tab) {
  homeTab.value = tab
  if (tab === 'aiops') router.replace('/aiops')
  else if (tab === 'alarm') router.replace('/overview')
...
async function fetchAlarmData() {
  ...
  try {
    ...
    if (incRes.success) {
      ...
      // Build app list from incidents
      const appMap = {}
      for (const inc of incRes.data) {
        for (const a of (inc.related_alerts || [])) {
          const res = a.resource || ''
          const name = res.replace(/\s*\(.*\)/, '').trim()
          if (!appMap[name]) appMap[name] = { name, type: a.category || '其他', status: a.level === 'critical' ? 'critical' : a.level === 'warning' ? 'warning' : 'normal', score: a.level === 'critical' ? '异常' : '正常', alertCount: 0 }
...
const openAlarmAnalysis = function(record) {
  const isCapacity = record.category === '容量类'
  if (isCapacity) {
    message.info('AI自动触发自愈流程：' + record.title)
  } else {
    router.push('/alarm/analysis/' + record.incident_no)
  }
}

const openAlarmDetail = function(record) {
  router.push('/alarm/analysis/' + record.incident_no)
}
...
/* ===== 告警分析 Tab ===== */
.alarm-analysis-page { display: flex; flex-direction: column; gap: 16px; padding-bottom: 24px; }
...
.aa-empty-text i { font-size: 24px; display: block; margin-bottom: 8px; color: #D9D9D9; }
